...
# Render assigns PORT, local uses 8501
STREAMLIT_PORT = os.environ.get('PORT', '8501')

# FastAPI runs on internal port
FASTAPI_PORT = '8000'

# Host configuration
HOST = '0.0.0.0' if IS_PRODUCTION else '127.0.0.1'

# FastAPI URL - USE LOCALHOST FOR SAME CONTAINER
if IS_PRODUCTION:
    # Same container = use localhost
    FASTAPI_URL = f"http://localhost:{FASTAPI_PORT}"
else:
    # Local development
    FASTAPI_URL = f"http://{HOST}:{FASTAPI_PORT}"

print(f"[CONFIG] IS_PRODUCTION: {IS_PRODUCTION}")
print(f"[CONFIG] FASTAPI_URL: {FASTAPI_URL}")
print(f"[CONFIG] STREAMLIT_PORT: {STREAMLIT_PORT}")
